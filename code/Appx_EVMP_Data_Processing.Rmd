...
sort.dbh2join <- dbh.perc %>%
  filter(timeClass == "BL") %>% # names() 
  group_by(timeClass, DBHclGp01, SITE_TYPE) %>% 
  dplyr::mutate(rank_dbhClass_BL = rank(percentTot, ties.method = "first")) %>% 
  ungroup() %>% 
  filter(timeClass == "BL") %>% 
  select(SITE_TYPE, SITE_ID, DBHclGp01, rank_dbhClass_BL) %>% 
  distinct() %>% 
  arrange(SITE_TYPE, DBHclGp01, rank_dbhClass_BL, SITE_ID) %>% 
  pivot_wider(names_from = DBHclGp01, 
              names_glue = "sort_{DBHclGp01}",
              values_from = rank_dbhClass_BL) %>% 
  clean_names() %>% 
  arrange(site_type, desc(sort_0_10_cm), site_id) %>% 
  select(2:5) %>% 
  distinct()
...
# AC  
plot.percTot.AC.F <- full_join(dbh.perc.clean,sort.dbh2join) %>% 
  filter(site_type == "AC" & fenced == "Fenced") %>% 
  ggplot(aes(reorder(site_id, -sort_0_10_cm), percent_tot)) +
  geom_col(aes(fill = dbh_class)) +
