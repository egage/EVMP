...
# ggsave("./output/figures_exported/rev_plotcnt_pTypeXvalley.png", width = 7, height = 3.75)
...
# ggsave(plot = pl.hm1, filename = "./output/figures_exported/descriptive_background/cnt_plot_valley_x_fenced.png", width = 5, height = 4, dpi = 300)
...
# ggsave(plot = pl.hm2, filename = "./output/figures_exported/descriptive_background/cnt_ptype_x_fenced.png", width = 5, height = 4, dpi = 300)
...
# ggsave(plot = cow1, filename = "./output/figures_exported/descriptive_background/cnt_ptype_valle_x_fenced.png", width = 5, height = 4, dpi = 300)
...
# ggsave("./output/figures_exported/Revised_WCWNCWK_LI_shrubHt_FenBur_boxplot.png", width = 8, height = 4.875)
...
# moving forward treating them as the second scenario for now...
```
...
canopy.all.willow.sum %>%
  write_csv("./output/exported_data/willow_macroplot_cover_BLto2018.csv") # re-exported 2/22/21
...
ggsave("./output/figures_exported/WCxWNC_willow_percentPlotCovThresh_dfc.png", width = 5.5, height = 2.25)
...
## I can not find in the SAS code where the areawt values are calculated


# create a tibble from the area wts in teh SAS code above
lz.wt.willow <- tribble(
...
```{r, echo=FALSE, eval=FALSE}
# not clear how area wts are calculated i noriginal SAS code
# the following is based on proportional plot count
...
wt.cov <- canopy.all.willow.sum %>% 
  filter(SITE_TYPE != "WK") %>% 
  group_by(timeClass, SITE_TYPE, BURNED, FENCED, zCond2) %>%
  summarise(n = n(), mean = mean(cover.allwillow, na.rm=TRUE), sd = sd(cover.allwillow, na.rm=TRUE)) %>%
  ungroup() %>% 
  arrange(timeClass, zCond2)

# join in weigts from 2015 SAS code
wt.cov <- left_join(wt.cov,lz.wt.willow) 

## weight
wt.cov <- wt.cov %>% 
  filter(timeClass %in% c("BL","2013","2018")) %>% 
  mutate(mean.wt = areawt_lz*mean) 
...
```{r, eval=FALSE, echo=FALSE}
canopy.all.willow.sum %>% 
 filter(timeClass == "BL" & zCond2 == "WC-BG") %>% 
  gt()
...
ggsave("./output/figures_exported/revised_WC_WNC_HtMeanDiff18BL_TCxfenced_dfc.png", width = 6.5, height = 3.75, dpi = 300)
...
ggsave("./output/figures_exported/revised_WC_WNC_HtMeanDiff18BL_TCxburned_dfc.png", width = 5.5, height = 3.75, dpi = 300)
...
ggsave("./output/figures_exported/willow_height/wilht_wc_wnc.png", width = 6.5, height = 3.75, dpi=300)
...
ggsave("./output/figures_exported/revised_figs_aspen/AC_ht_stemsAc_tile_revised.png", width = 9.75, height = 10.5)
...
asp.tally.dbh.tidy %>%
  ...
  ungroup() %>% 
  ggplot(aes(timeClass, DBHclGp01)) +  
  geom_tile(aes(fill = mean.tally), color = "white", alpha = .4) +
  # geom_tile(fill = "grey90", color = "white", alpha = .2) +
  geom_text(aes(label = mean.tally), size = 3) +
  facet_wrap(~FENCED) +
  theme_minimal() +
  labs(x = "", y = "", fill = "Mean tally", title = "Mean aspen count in burned plots", caption = "Plots burned in 2012, AC&ANC, live stems only") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

ggsave("./output/figures_exported/revised_figs_aspen/asp_burned_live_only_TCxF_mean.png", width = 6, height = 4)
...
ggsave("./output/figures_exported/revised_figs_aspen/asp_burned_only_TCxF_LiveXdead_meanCnt.png", width = 7, height = 5)
...
ggsave("./output/figures_exported/revised_figs_aspen/AC_diam_stemsAc_tile_rev.png", width = 7.5, height = 7.5)
```
...
ggsave("./output/figures_exported/revised_figs_aspen/ANC_diam_stemsAc_tile_rev.png", width = 7.5, height = 5.5)
...
ggsave("./output/figures_exported/revised_figs_aspen/AK_diam_stemsAc_tile_revised.png", width = 5.5, height = 3.5)
...
ggsave("./output/figures_exported/revised_figs_aspen/AspPercTotXdbh_ac_FxUf.png", width = 10, height = 5, dpi=300)
ggsave("./output/figures_exported/revised_figs_aspen/AsppercTotXdbh_ac_FxUf.pdf", width = 10, height = 5)
...
ggsave("./output/figures_exported/revised_figs_aspen/AspPercTotXdbh_anc.png", width = 7, height = 5, dpi=300)
ggsave("./output/figures_exported/revised_figs_aspen/AsppercTotXdbh_anc.pdf", width = 7, height = 5)
...
ggsave("./output/figures_exported/revised_figs_aspen/AspPercTotXdbh_ak.png", width = 5, height = 5, dpi=300)
ggsave("./output/figures_exported/revised_figs_aspen/AsppercTotXdbh_ak.pdf", width = 5, height = 5)
...
ggsave("./output/figures_exported/revised_figs_aspen/AspPercTotXsuckeringClass_ac_anc.png", width = 5, height = 5, dpi=300)
ggsave("./output/figures_exported/revised_figs_aspen/AspPercTotXsuckeringClass_ac_anc.pdf", width = 5, height = 5)
...
## save revised figure
ggsave("./output/figures_exported/aspen_stem_count/AC_ANC_suckering_1p5to2p5m_fenced.png", width = 7.5, height = 5)
...
plot.percTot.AC.F <- full_join(ht.perc01,sort.ht2join) %>% 
  ...
  scale_fill_viridis(discrete=TRUE) +
  theme_minimal() +
  labs(y = "% of total stems", x = "", fill = "Height class", title = "Proportion of Live Aspen Stems by Height Class", subtitle = "Fenced plots") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_y_continuous(labels=scales::percent) +
  theme(legend.position = "none")
...
ggsave("./output/figures_exported/revised_figs_aspen/AspPercTotXhtclass_ac_FxUf.png", width = 10, height = 5, dpi=300)
ggsave("./output/figures_exported/revised_figs_aspen/AsppercTotXhtclass_ac_FxUf.pdf", width = 10, height = 5)
...
plot.percTot.ANC <- full_join(ht.perc01,sort.ht2join) %>% 
  ...
  scale_fill_viridis(discrete=TRUE) +
  theme_minimal() +
  labs(y = "% of total stems", x = "", fill = "Height class", title = "Proportion of Live Aspen Stems by Height Class", caption = "Noncore Winter Range") +
  # labs(y = "% of total stems", x = "", fill = "Height class", title = "Proportion of Live Aspen Stems by Height Class", subtitle = "Unfenced plots only") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
  scale_y_continuous(labels=scales::percent)
...
ggsave("./output/figures_exported/revised_figs_aspen/AspPercTotXhtclass_anc_FxUf.png", width = 7, height = 5, dpi=300)
ggsave("./output/figures_exported/revised_figs_aspen/AspPercTotXhtclass_anc_FxUf.pdf", width = 7, height = 5)
...
ggsave("./output/figures_exported/revised_figs_aspen/AspPercTotXhtclass_ak.png", width = 5, height = 5, dpi=300)
ggsave("./output/figures_exported/revised_figs_aspen/AspPercTotXhtclass_ak.pdf", width = 5, height = 5)
...
asp.sign.summary %>% 
  ...
  labs(x="", y = "Percent of plots", fill = "", caption = "2018 Data") 

ggsave("./output/figures_exported/revised_figs_aspen/asp_animal_presence_2018.png", width=6, height = 5, dpi=300)
ggsave("./output/figures_exported/revised_figs_aspen/asp_animal_presence_2018.pdf", width=6, height = 5)
...
asp.sign.summary.b %>% 
  ...
  scale_y_continuous(labels=scales::percent) +
  scale_fill_manual(values = c("grey20", "grey70")) +
  labs(x="", y = "Percent of plots", fill = "", caption = "2018 Data. Burned refers to 2012, not ET fire. Labels = count of plots") 

ggsave("./output/figures_exported/revised_figs_aspen/asp_animal_presence_2018_burned.png", width=6.5, height = 5, dpi=300)
ggsave("./output/figures_exported/revised_figs_aspen/asp_animal_presence_2018_burned.pdf", width=6.5, height = 5)
...
-   All formatted data for 2018 can be found under
    EVMPVegetationAnalysis10 Year Review
...
-   Created new shapefiles for all veg plots; trying to find shapefiles
    ...
    EVMPVegetationGISEVMP_Veg_Data_2018.gdb OR
    ShapefilesMaster_Veg_Sites_Shp2018

-   Created new veg maps. Located under
    EVMPVegetationLogisticsMapsEVMP vegetation plot maps2018 Veg
    Site MapsPlot Maps

-   Wild Basin Willow Data -- I created a folder with all of the data
    collected at Wild Basin this fall. It is located under
    EVMPVegetationAnnual RecordsFY18Wild Basin -- Willow Data
...
```{r, echo=FALSE, eval=FALSE}
# **References**   #  # Johnson, T. L., L. C. Zeigenfuss, N. Thompson, and J. A. Mack. 2016. The role of science through a century of elk and habitat management at Rocky Mountain National Park. Park Science 32(2):70–72 #  # U.S. Department of the Interior, 2007, Elk and vegetation management plan, Rocky Mountain National Park, Colorado: Washington D.C., National Park Service, unpaged, accessed online May 6, 2013 at http://www.nps.gov/romo/parkmgmt/elkveg_mgmt_plan_feis.htm. #  # U.S. Department of the Interior, 2008, Record of decision, Final environmental impact statement, Elk and vegetation management plan, Rocky Mountain National Park, Colorado: Washington D.C., National Park Service, 21 p., accessed online May 6, 2013 at www.nps.gov/romo/parkmgmt/upload/rod_evmp_signed_2-15-08.pdf. #  # Zeigenfuss, L., Johnson, T., and Wiebe, Z., 2011, Monitoring plan for vegetation responses to elk management in Rocky Mountain National Park: U.S. Geological Survey Open-File Report 2011–1013, 85 p. #  # Zeigenfuss, L.C., and T.L. Johnson, 2015, Monitoring of vegetation response to elk population and habitat management in Rocky Mountain National Park, 2008–14: U.S. Geological Survey Open-File Report 2015–1216, 44 p., http://dx.doi.org/10.3133/ofr20151216.
