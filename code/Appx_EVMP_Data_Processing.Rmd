...
ggTile_yr_season_site2 <- function(df){
  ...
  scale_fill_gradientn(values = c("#9ADCBB", "#FCFFDD")) +
...
pl.hm1 <- site.info.clean.sf %>%
  ...
  theme_minimal() +
  scale_fill_gradientn(colours = colfunc1(5)) +
  # scale_fill_viridis() +
  facet_wrap(~pType) +
...
csv.all.lc.li.df %>%
  ...
  scale_fill_manual(values = colfunc1(2)) +
  # viridis::scale_fill_viridis(discrete = TRUE, option = "D") +
  theme_minimal() +
  # facet_wrap(~SITE_TYPE, ncol = 1) +
...
tmp2 <- canopy.all.willow.sum  %>% 
  ...
  mutate(plots = map2(.y = LOCATION, .x = data, ~{ggplot(data = .x) +
                              geom_boxplot(aes(y = cover.allwillow, x = timeClass), fill="Gray40") +
                              # ggtitle(paste0("Willow cover, site: ", .y)) +
...
# ggsave("./output/figures_exported/density_percleaderuse_wnc.png", width = 4.5, height = 3.75, dpi = 300)
...
ggsave("./output/figures/asp_MP_deadTally.png", width = 9.5, height = 7, dpi = 300)
...
combo.acanc <- f.plot1 + uf.plot1 + plot_layout(ncol = 1, heights = c(1, 3))
combo.acanc

# save plot
ggsave("./output/figures_exported/acanc_prop_height_raw_revised.png", width = 7.5, height = 12, dpi= 300)
...
# ggsave("./output/figures_exported/AC_Live_Fenced_heightClass_heatmap3b.png", width = 8.5, height = 4, dpi = 300)
