...
This document provides a basic exploration of EVMP files provided by RMNP on October 10, 2018.
...
The following are notes from E. Ertl describing EVMP vegetation data collection, as recorded in 2018.  
...
* Formatted 2015 – 2018 data  

* 2015-2017 data only consisted of burn plot sampling; no burn plot data collected in 2014
...
* Fixed unit and rounding issues in data.
	In 2016 the crew measured additional burned sites that are not on the burn survey list (WC32, WC34, WC78, WC84, and WC85). For some plots they only measured willow instead of measuring all shrubs. Fixed unit and rounding issues in data.

* There is only west side data for 2016 and 2017; Nick B. said they weren’t aware that west side plots were supposed to be measured every year back in 2014/2015
	Burn survey and west side data are currently combined by year measured
...
* Protocols are currently being updated for 2018; Hopefully this will clear up data inconsistencies the last few years 
* Master databases have been updated for 2018 
...
* Created new veg maps
	Located under EVMPVegetationLogisticsMapsEVMP vegetation plot maps2018 Veg Site MapsPlot Maps
...
```{r}
#### maps
### choose either from the z or non-z containing vars. I'm not sure which?? Seem equivalent as far as count
...
sinfo.sf <- st_as_sf(sinfo.df, coords = c("UTM_E_NAD83", "UTM_N_NAD83"), crs = 26913)
...
## test function. selects only the DATE column
...
csv.all.lc.li.df %>% 
  filter(!is.na(MAX_HEIGHT_CM)) %>%
  # names() %>% 
  group_by(yr,SITE_TYPE, SITE_NUMBER, SITE_ID, SPECIES_CODE, MAX_HEIGHT_CM) %>%
...
csv.all.lc.li.df %>% 
  ...
  filter(SPECIES_CODE !="SAPE" & SPECIES_CODE !="SAWO" & SPECIES_CODE !="SAXX" & SPECIES_CODE !="SALUC" & SPECIES_CODE !="SABR") %>%
  # names() %>% 
  # group_by(yr,SITE_TYPE, SPECIES_CODE, MAX_HEIGHT_CM) %>%
  # summarise(mean.max.ht = mean(MAX_HEIGHT_CM,na.rm=TRUE)) %>% 
...
Monitoring plan for vegetation responses to elk management in Rocky Mountain National Park; 2011; OFR; 2011-1013 

Zeigenfuss, Linda C.; Johnson, Therese; Wiebe, Zachary. Alpine plant community trends on the elk summer range of Rocky Mountain National Park, Colorado: an analysis of existing data; 2006; OFR; 2006-1122; 

Zeigenfuss, Linda C. Effects of elk herbivory on vegetation and nitrogen processes; 2004; Article; Journal; Journal of Wildlife Management; Schoenecker, K.A.; Singer, F.J.; Zeigenfuss, L.C.; Binkley, D.; Menezes, R.S.C. Ecological evaluation of the abundance and effects of elk herbivory in Rocky Mountain National Park, Colorado, 1994-1999; 2002; OFR; 2002-208; Compiled by Singer, Francis J.; 

Zeigenfuss, Linda C. Influences of herbivory and water on willow in elk winter range 2002 Journal of Wildlife Management 

Zeigenfuss, L.C.; Singer, F.J.; Williams, S.A.; Johnson, T.L. Sustainability of vegetation communities grazed by elk in Rocky Mountain National Park; 2002; Report; Report; Ecological evaluation of the abundance and effects of elk herbivory in Rocky Mountain National Park, Colorado, 1994-1999. Open File Report 02-208; Schoenecker, K.A.; Singer, F.J.; Menezes, R.S.C.; Zeigenfuss, L.C.; Binkley, D.
