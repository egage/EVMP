...
##### All willow, site type, fenced/unfenced
...
##### All willow, site type, fenced/unfenced, burned/unburned
...
csv.all.lc.li.df %>% 
  ...
  theme_minimal() +
  # scale_fill_manual(values = c("ivory2","lightblue")) +
  labs(title = "Core winter range: maximum shrub height, fencing, burning", x = "Year", color = "", shape = "", y = "Height (cm)", caption = "Pooled SAMO,SAPL, SAGE, SABE,SADR,SAPE,SAER")  +
  facet_grid(~BURNED)
facet_wrap(~BURNED)
...
# visdat::vis_dat(canopy.all.willow.sum)
```
...
canopy.all.willow.sum.sf %>%
  filter(timeClass == "2018") %>% 
  mapview(zcol='cover.allwillow')
...
See Weighting Scheme used by LZ
...
# ggsave("./output/figures_exported/All_winter_range_willow_cov_boxplot_2panel.png", width = 6.5, height = 3.75)
```
...
canopy.all.willow.sum %>% 
  names() %>% 
  select()
left_join((filter(site.info.clean.sf, pType == "willow")),select(canopy.all.willow.sum, c(SITE_ID,canopy.all.willow.sum, cover.allwillow)))
...
allshrub.mcro.all %>% 
  ...
  theme_minimal() +
  ylim(0,320) +
  labs(x = "", y = "Shrub height (cm)", fill = "", caption = "WC only, all shrubs")

# ggsave("./output/figures_exported/WC_allshrub_ht_boxplot.png", width = 5.5, height = 3.5)
...
# ggsave("./output/figures_exported/WC_willow_ht_boxplot.png", width = 5.5, height = 3.5)

# ggsave("./output/figures_exported/WC_allnonwillow_ht_boxplot.png", width = 5.5, height = 3.5)
```
...
willow.mcro.all %>%
  ...
  ylim(0,350) +
  theme_minimal() +
  labs(x = "", y = "Willow height (cm)", fill = "", caption = "Macroplot, all willow, WC only") +
  facet_wrap(~time_class)

# ggsave("./output/figures_exported/WC_willow_ht_boxplot_site_type.png", width = 5.5, height = 3.5)
...
ggsave("./output/figures_exported/WNC_willow_ht_boxplot.png", width = 5.5, height = 3.5)
...
# ggsave("./output/figures_exported/WK_willow_ht_boxplot.png", width = 5.5, height = 3.5)
...
# ggsave("./output/figures_exported/AC_ht_stemsAc_tile_d.png", width = 9.75, height = 7.5)
...
# ggsave("./output/figures_exported/ANC_diam_stemsAc_tile_b.png", width = 7.5, height = 5.5)
...
# ggsave("./output/figures_exported/AK_diam_stemsAc_tile_c.png", width = 5.5, height = 3.5)
...
# ggsave("./output/figures_exported/AC_Live_Fenced_PropDBH_bc.png", width = 8, height = 6, dpi = 300)
...
suckering.vtype.burned.allHt <- ht.dbh.class %>%
    # filter(HTclass == "HT_151_200_CM" |HTclass == "HT_201_250_CM") %>%
  group_by(SITE_ID, HTclass.lbl, RANGE_TYPE, FENCED, BURNED, timeClass) %>% 
  summarise(stemTally.htclass = sum(stemTally)) %>%
  ungroup() %>% 
...
# ggsave("./output/figures_exported/suckering_proportion_all_plots_ht_class_bw.png", width = 7, height = 3.85)
...
# ggsave("./output/figures_exported/suckering_proportion_all_plots_ht_class_rangety_bw.png", width = 8, height = 3.85)
...
# ggsave("./output/figures_exported/AC_sum_dead_fenced_burned.png", width = 4.75, height = 3.5)
...
# ggsave("./output/figures_exported/asp_AC_liveTally.png", width = 8.5, height = 8, dpi = 300)
...
# ggsave("./output/figures_exported/ac_liveTally.png", width = 9.125, height = 6.5, dpi = 300)
...
# ggsave("./output/figures_exported/anc_liveTally.png", width = 7.125, height = 5.5, dpi = 300)
...
asp.tally.ht.tidy %>%
  ...
  scale_fill_viridis_c(begin = .4, end = .95, direction = -1, option = "B") +
  theme_minimal() +
  labs(x = "Height class", y = "", fill = "% total stems", title = "Proportion of Live Aspen Stems by Height Class", subtitle = "All plots")+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
asp.prop.f  
# ggsave("./output/figures_exported/AC_Live_Fenced_heightClass_bc.png", width = 8.5, height = 4.5, dpi = 300)
...
# ggsave("./output/figures_exported/AC_Live_unFenced_heightClass_bc.png", width = 6.5, height = 5.5, dpi = 300)
...
######### DEAL WITH NA #############
## Note
na_strings <- c(".", "na", "NA", "N A", "N / A", "N/A", "N/ A", "Not Available", "NOt available","0")
# 
...
offt.cln <- left_join(offt.cln, site.info.clean, by = "site_id")
...
offt.percbrowsed <- offt.cln %>% 
  ...
  select(c(yr, site_type, site_id, species_code, sum_bu_per_stem, percent_bu_per_stem, fenced))
...
upl.allspp.cover %>% 
  ...
  theme_minimal() +
  labs(x = "", y = "% shrub cover", fill = "", caption = "Upland cover, all shrubs combined")

# ggsave("./output/figures_exported/upland_species_cover_allspp.png", width = 4.5, height = 3.75, dpi = 300)


upl.allspp.cover.summary <- upl.allspp.cover %>% 
  group_by(yr, site_type) %>%
  summarytools::descr() %>% 
  summarytools::tb() %>% 
