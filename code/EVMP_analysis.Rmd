...
This is a working document aimed at facilitating data QA/QC and analysis. 
...
library(flextable)
# remotes::install_github("rstudio/gt") # Rstudio alternaitive to flextable?
library(gt)
...
In 2008, Rocky Mountain National Park (RMNP) began implementation of the Elk and Vegetation Management Plan (EVMP), intended to guide management of elk and vegetation over a 20–yr period, reduce the impacts of elk on vegetation, and bring  target ecosystems into the natural range of variability for elk and vegetation in three target vegetation communities: aspen, montane riparian willow, and upland. Management includes fencing, non-lethal redistribution and culling of elk herds, and active vegetation restoration, the plan incorporates the principle of adaptive management to guide management. Achieving the goals of the EVMP requires monitoring of target vegetation communities. 
...
This report analyzes data from the first 10 years of monitoring aspart of the EVMP and will be used to assess progress towards program goals. Monitoring in the EVMP program is focused on assessing several community attributes (Zeigenfuss et al. 2011) including:
...
Protocols detailed in the EVMP monitoring plan (Zeigenfuss et al. 2011) were used to collect vegetation data at study plots. A round of baseline measurements collected between 2006–2009 were made, with RMNP staff collecting subsequent data. A subsample of plots were visited annually beginning in 2007 for upland plots and 2009 willow plots and conducted through 2018. Offtake measures on willows were usually made in May and June, but delayed to mid-July in some years due to weather. Other variables were assessed the year of plot establishment and during June through September in 2013 and 2018.
...
site.info.all %>% 
  ...
  select(-WILDERNESS) %>% 
  gt::gt() %>%
  tab_header(title = "EVMP plots listed as 'Removed' or lacking valid coordinates")
...
vt <- st_read("./data/EVMP_derived/site_info_all_sf.shp")
vt <- vt %>% 
...
us <- c(left = -105.99, bottom = 40.134, right = -105.4, top = 40.55)
get_stamenmap(us, zoom = 10, maptype = "toner-lite") %>% ggmap() 

## google maps just wants the center
rmnpc <- c(-105.696185, 40.304694)

ggmap::get_googlemap(center = rmnpc, zoom = 10, scale = 2, format = "png8", maptype = "terrain")

ggmap::get_googlemap(c(-97.14667, 31.5493))
ggmap(map)
40.288983, -105.682452

get_googlemap(us, zoom = 10, maptype = "roadmap") %>% ggmap() 
...
site.info.all %>%
  dplyr::select(-REMOVED) %>%
  group_by(VALLEY, pType) %>% 
  tally() %>% 
  gt::gt() %>% 
  tab_header(title = md("**EVMP plots with an 'NA' in the 'Removed' field**")) 
gt.siteinfo
...
pl.cnt1 <- site.info.all.sf %>%
  ...
  # scale_x_log10() +
  theme_minimal() +
  labs(x="Range type", y = "Count", title = "Count of EVMP sampling sites") +
  coord_flip()
...
** Records for which there is "NA" for "VALLEY"**
...
```{r}
#### There are two duplicate sets of files, one with a 'z' in the name, one without, but otherwise seeming the same. In this chunk, picking one set (the zed set)
...
**Missing data: line intercept by year.**
...
**Missing data: line intercept by month.** 
...
### Master excel file: Upland_Line_Intercept_2007_2013_2018.xlsx
...
csv.all.lc.li.df %>% 
  ...
  # distinct(yr)
  # glimpse() %>% 
  # distinct(SITE_TYPE,SITE_ID, FENCED) %>%
  filter(!is.na(FENCED)) %>% 
  filter(!is.na(MAX_HEIGHT_CM)) %>%
  filter(FENCED != "Y_but_fence_down_since_2013") %>% 
  filter(SPECIES_CODE != "SAXX") %>%
...
#### Willow height: _Salix moniticola_ 
```{r}
...
#### Willow height: _Salix planifolia_
...
csv.all.lc.li.df %>%
  ...
  filter(SPECIES_CODE == "SAPL") %>%   filter(!is.na(MAX_HEIGHT_CM)) %>%
  filter(str_detect(SPECIES_CODE, "^SA")) %>% 
  filter(SPECIES_CODE !="SAPE" & SPECIES_CODE !="SAWO" & SPECIES_CODE !="SAXX" & SPECIES_CODE !="SALUC" & SPECIES_CODE !="SABR"& SPECIES_CODE !="SAER") %>%
  filter(str_detect(SPECIES_CODE, "^SA")) %>%
  ggplot() +
  ...
  theme_minimal() +
  facet_wrap(~SITE_TYPE, ncol = 1) +
  labs(x = "Max height (cm)", y = "Year", caption = "SAPL") +
  coord_flip()
...
csv.all.lc.mcro.df %>% 
  ...
  filter(BEAVER_PRESENCE != "Absence_") %>%
  # distinct(SITE_ID)
  group_by(SITE_ID, BEAVER_PRESENCE, yr) %>% 
  tally() %>% 
  dplyr::select(-n) %>% 
  datatable()
...
csv.all.lc.mcro.df %>% 
  # distinct(MOOSE_PRESENCE)
  group_by(SITE_ID, MOOSE_PRESENCE, yr, LOCATION) %>% 
  filter(MOOSE_PRESENCE != "Absence_") %>%
  tally() %>% 
  dplyr::select(-n) %>% 
  ungroup() %>%
  group_by(yr, MOOSE_PRESENCE, LOCATION) %>% 
  tally() %>% 
  ungroup() %>%
  mutate(yr = as.character(yr)) %>% 
  ggplot(aes(yr, MOOSE_PRESENCE)) +
...
### keep amd discard
# asp.d %>% keep(.x = .,)
# asp.d %>% map(., discard("Aspen 2013 Site Info"))
...
asp.d.sel.comb <- bind_rows(asp.d.sel, .id = "tab") 
...
tl1 %>% 
  colformat_num(.,col_keys = names(digits = 1)
...
# asp.d %>% 
#   # names()
#   pluck("Aspen Baseline Stem Tallies") %>% 
#   # pluck("Aspen 2017 Stem Tallies (burn)") %>% 
#   names() %>% 
#   enframe() %>% 
#   flextable()
