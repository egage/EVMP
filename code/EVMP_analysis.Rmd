...
This code is a working document, aimed at facilitating data QA/QC and exploratory analysis. 
...
Since its adoption in 2008, Rocky Mountain National Park (RMNP) has been implementing the Elk and Vegetation Management Plan (EVMP), which will guide management over a 20–yr period and aims to reduce the impacts of elk on vegetation and return systems to the natural range of variability for elk and vegetation in three target vegetation communities: aspen, montane riparian willow, and upland herbaceous. Relying on different conservation tools such as fencing, non-lethal redistribution of elk, vegetation-restoration, and culling of elk herds, the plan incorporates the principle of adaptive management to guide management actions. Adjusting management to achieve the EVMP’s objectives requires monitoring of target vegetation communities. 

RMNP and the U.S. Geological Survey (USGS) developed a vegetation monitoring program to allow park managers to assess progress towards goals of the EVMP. Over 200 monitoring sites were established and baseline data collected from 2006-2009 in core winter range aspen, willow, and upland communities. Annual sampling of offtake by ungulates and a variety of structural and compositional metrics has occurred on a five-year basis, with a second five-year-round of sampling completed in 2018. 

This document provides an overview of ongoing analyses of these data, comparing datasets collected in willow, aspen, and upland herbaceous plots in core and non-core elk winter range site on the east side of RMNP and the Kawunechee Valley. Results compare trends in attributes like stem count, diameter, and shrub height over time and in relation to factors like study site and whether plots were in fenced or unfenced areas. In addition, analyses evaluate effects of a 2012 wildfire that burned over Moraine Park, which provided a unique opportunity to study willow and aspen recovery after fire in both grazed and ungrazed conditions.

### Objectives 

The goal of this project is to provide data analysis to RMNP for the 10-year interval, including analysis of data from the plots burned in the 2012 fire, to ensure timely and responsive adaptive management of elk and vegetation on the park’s winter range. This document provides a basic exploration of EVMP files provided by RMNP in October of 2018.  The code and analyses aim to characterize basic data structure, identify problems, and clean and reorganize as needed for plotting and modeling. Specific objectives include identifying issues such as:

* Inconsistently named/typed factors    
* Missing values
* Data values outside of expected range or showing unusual patterns

Cleaned and archivable data sets will be produced to facilitate future data analysis and sharing.

# Methods

The following code cleans and analyzes data collected since the inception of the EVMP.
...
Select among the tabs below to view data import steps and basic data characteristics.
...
cowplot::plot_grid(pl.cnt1, pl.cnt2, labels = "AUTO")
...
csv.all.lc.li.df %>% 
  ...
  theme_bw() +
  labs(title = "Maximum shrub height", subtitle = "Mean of shrub height pooled by site", x = "Year", y = "Species") +
  facet_wrap(~FENCED, ncol= 3)
