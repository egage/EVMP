...
site.info.clean <- site.info.clean %>% 
  mutate(FENCED = if_else(is.na(FENCED),
                          "N", FENCED)) %>% 
...
pl.cnt2 <- site.info.clean.sf %>%
  ...
  labs(x="", y = "Count", title = "Count of EVMP sampling sites by 'Range type'")
...
## join in site info from the master plot list
...
mcro.canopy.diam.sum <- csv.all.lc.mcro.df %>%
  ...
  summarise(cano_ht_med = median(CANOPY_DIAM_AVG)) 
