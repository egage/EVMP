...
## Aspen Density, Comparison of Sapling Classes
...
## Aspen Density, Short Saplings (<=1.5 m) - Combined Winter Range (AC+ANC)
```{r}
...
mcmc_areas(posteriors.acanc.stcnt.sm,
           prob = 0.9) +
  ggtitle("Posterior distributions",
                      "with medians and 90% credible intervals")
```
...
fun.sexit.gt(stmod_stally2.acanc.sm) %>%
 tab_header(title = "Aspen Count Posterior Description", subtitle = "Small Saplings, All Winter Range") %>%
...
posteriors.acanc.stcnt.sm.tidy %>% 
  ...
  # scale_fill_manual(values = colfunc2(ncol(posteriors.acanc.stcnt.sm.tidy))) +
...
# Visualize the pd
#### PD - AC
p_direction(stmod_stally2.acanc.sm) %>% 
  plot() +
  theme_minimal() +
  geom_vline(aes(xintercept = 0), color= "black", size=1, lty="dashed") +
  labs(caption = "Aspen stem count, Small Saplings, All Winter Range")
...
# HPD interval probability: 0.95, Results on log scale", x = "Estimate", y = "Contrast")
  labs(subtitle = "Point estimate displayed: median HPD interval probability: 0.95", x = "Estimate", y = "Contrast", caption = "All Winter Range, Aspen Stem Count, Small Saplings. Results are given on the log scale")
...
## Aspen Density, Short Saplings (<=1.5 m) - Core Winter Range (AC)
```{r}
asp.sm.ac <- asp.sm.ac %>%
  mutate(stem_den_ha = stem_den_ha + 0.0000001) 
```
...
```{r}
color_scheme_set("teal")
mcmc_areas(posteriors.ac.stcnt.sm,
           prob = 0.9) +
  ggtitle("Posterior distributions",
                      "with medians and 90% credible intervals")
```
...
## Aspen Density, Short Saplings (<=1.5 m) - Noncore Winter Range (ANC)
...
```{r}
color_scheme_set("teal")
mcmc_areas(posteriors.anc.stcnt.sm,
           prob = 0.9) +
  ggtitle("Posterior distributions",
                      "with medians and 90% credible intervals")
```
...
```{r}
#### Contrasts
# EMMs for later fanctor levels are subtracted from those for earlier levels; if you want the comparisons to go in the other direction, use pairs(pigs.emm.s, reverse = TRUE).
...
## Aspen Density, Short Saplings (<=1.5 m) - Kawuneeche Valley (AK)
...
mcmc_areas(posteriors.ak.stcnt.sm,
           prob = 0.9) +
  ggtitle("Posterior distributions",
                      "with medians and 90% credible intervals") +
  theme_minimal()
...
glimpse(posteriors.acanc.stcnt.sm)
posteriors.acanc.stcnt.sm %>% 
  rename('2013' = time_class2013) %>%
  rename('2018' = time_class2018) %>%
  rename(Unfenced = fencedUnfenced) %>%
  rename('2013:Unfenced' = `time_class2013:fencedUnfenced`) %>%
  rename('2018:Unfenced' = `time_class2018:fencedUnfenced`) %>%
  mcmc_areas(.,
           prob = 0.9) +
  ggtitle("All Winter Range") +
  theme_minimal() +
...
# combine
smsap.plot1 + smsap.plot2 + smsap.plot3 + smsap.plot4
...
## Aspen Density, Tall Saplings (1.5-2.5 m ht) - Combined Winter Range (AC+ANC)
...
```{r}
color_scheme_set("teal")
mcmc_areas(posteriors.acanc.stcnt.lg,
           prob = 0.9) +
  ggtitle("Posterior distributions",
                      "with medians and 90% credible intervals")
```
...
fun.sexit.gt(stmod_stally2.acanc.lg) %>%
 tab_header(title = "Aspen Count Posterior Description", subtitle = "Large Saplings, All Winter Range") %>%
...
p_direction(stmod_stally2.acanc.lg) %>% 
  ...
  theme_minimal() +
  geom_vline(aes(xintercept = 0), color= "black", size=1, lty="dashed") +
  labs(caption = "Aspen stem count, Large Saplings, All Winter Range")
...
emmeans(stmod_stally2.acanc.lg, ~ time_class | fenced) %>% 
  ...
  # labs(title= "Core winter range", caption = "Point estimate displayed: median 
# HPD interval probability: 0.95, Results on log scale", x = "Estimate", y = "Contrast")
  labs(subtitle = "Point estimate displayed: median HPD interval probability: 0.95", x = "Estimate", y = "Contrast", caption = "All Winter Range, Aspen Stem Count, Large Saplings. Results are given on the log scale")
...
## Aspen Density, All Stem Diameters - Combined Winter Range (AC+ANC)
```{r}
# purrr plots
asp.acanc  %>% 
...
p_direction(stmod_stally2.acanc) %>% 
  ...
  theme_minimal() +
  geom_vline(aes(xintercept = 0), color= "black", size=1, lty="dashed") +
  labs(caption = "Aspen stem count, all winter range")
...
emmeans(stmod_stally2.acanc, ~ time_class | fenced) %>% 
  ...
  # labs(title= "Core winter range", caption = "Point estimate displayed: median 
# HPD interval probability: 0.95, Results on log scale", x = "Estimate", y = "Contrast")
  labs(subtitle = "Point estimate displayed: median HPD interval probability: 0.95", x = "Estimate", y = "Contrast", caption = "All Winter Range, Aspen Stem Count, Results are given on the log scale")
...
## Aspen Density, All Stem Diameters - Core Winter Range (AC)
...
## Aspen Density, All Stem Diameters - Noncore Winter Range (ANC)
...
## Aspen Density, All Stem Diameters - Kawuneeche Valley (AK)
