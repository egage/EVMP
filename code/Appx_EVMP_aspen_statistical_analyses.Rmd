...
## working..
# do not run
summary.fun <- function(data, var){
  summarytools::descr(data$var) %>% 
  tb() %>% 
  mutate(across(where(is.numeric), round, 1))
}

summary.fun(ht_perc1,stem_den_ac)
ht_perc1  %>% 
  group_nest(range_type) %>% 
  mutate(summary = map(.x=data, descr(stem_den_ac) %>% tb()))
  mutate(plots = map2(.y = range_type, .x = data, ~{ggplot(data = .x) +
                              geom_boxplot(aes(y = stem_den_ac, x = time_class), fill="Gray80") +
...
## Aspen Stem Count, All Stem Diameters - Combined Winter Range (AC+ANC)

```{r}
# purrr plots
ht_perc1  %>% 
  filter(site_type == "AC" | site_type == "ANC") %>% 
  group_nest(db_hcl_gp01) %>%
  # mutate(tbls = map(.x = data, .f = gt)) %>% 
  mutate(plots = map2(.y = db_hcl_gp01, .x = data, ~{ggplot(data = .x) +
      geom_boxplot(aes(y = stem_den_ac, x = time_class, fill=fenced.long), color="Gray80") +
      geom_jitter(aes(y = stem_den_ac, x = time_class, color=fenced.long), alpha=0.25) +
      labs(title = paste0("Fencing: ", .y), x = "", y = "Stem density (stems/acre)") +
      theme_minimal()})) %>% 
  pull(plots)
```
...
## Aspen Stem Count, All Stem Diameters - Core Winter Range (AC)
...
## Aspen Stem Count, All Stem Diameters - Noncore Winter Range (ANC)
...
## Aspen Stem Count, All Stem Diameters - Kawuneeche Valley (AK)
...
## Aspen Stem Count, Small Diamter Stems Only - Combined Winter Range (AC+ANC)
...
### Modeling and Posterior Description  
```{r}
asp.acanc.small <- asp.acanc %>%
  mutate(stem_den_ha = stem_den_ha + 0.0000001) %>% 
  filter(db_hcl_gp01=="0-10 cm") %>% 
  filter(live_dead == "LIVE")
```

```{r}
#### main factors + interactions:
stmod_stally2.acanc.small <- stan_glmer(stem_tally ~ time_class * fenced + (1 | site_id), data = asp.acanc.small,
                      family= poisson,
                      iter = 8000,
                      seed = 1234
                      )
...
**Prior summary**: stem_tally ~ time_class * fenced + (1 | site_id)  
```{r}
prior_summary(stmod_stally2.acanc.small)
```

```{r}
## model summary
fun.model.summary(stmod_stally2.acanc.small)
```

```{r}
# extract posteriors
posteriors.acanc.stcnt.small <- insight::get_parameters(stmod_stally2.acanc.small)

# tidy
posteriors.acanc.stcnt.small.tidy <- posteriors.acanc.stcnt.small %>%
  pivot_longer(cols = starts_with("time"), names_to = "parameter", values_to = "values")
...
color_scheme_set("darkgray")
mcmc_areas(posteriors.acanc.stcnt.small,
           prob = 0.8) +
  ggtitle("Posterior distributions",
                      "with medians and 80% credible intervals")
```
