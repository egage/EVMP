...
```{r}
# extract posteriors
posteriors.upl.putrubcov <- insight::get_parameters(stmod_cov.putr)

# tidy
posteriors.upl.putrubcov.tidy <- posteriors.upl.putrubcov %>%
  pivot_longer(cols = starts_with("time"), names_to = "parameter", values_to = "values")
...
```{r}
posteriors.upl.putrubcov.tidy %>%
  ggplot() +
  geom_density(aes(fill = parameter, x = values),color="grey", alpha = .25) +
  # scale_fill_viridis(discrete = TRUE) +
  scale_fill_manual(values = colfunc2(ncol(posteriors.upl.putrubcov.tidy))) +
  theme_minimal() +
  geom_vline(aes(xintercept = 0), color= "black", size=1, lty="dashed") +
...
### Modeling and Posterior Description
```{r, cache=TRUE}
upl.putr.uc <- upl.putr %>% 
  filter(site_type == "core range") # add 

## add cover in 0-1 range
upl.putr.uc <- upl.putr.uc %>%
  mutate(cover = perc_cover/100)
...
```{r}
# extract posteriors
posteriors.upl.putrubcov.uc <- insight::get_parameters(stmod_cov.putr.uc)
...
### Modeling and Posterior Description
```{r, cache=TRUE}
upl.putr.unc <- upl.putr %>% 
  filter(site_type == "non-core range") # add 

## add cover in 0-1 range
upl.putr.unc <- upl.putr.unc %>%
  mutate(cover = perc_cover/100)
...
```{r}
# extract posteriors
posteriors.upl.putrubcov.unc <- insight::get_parameters(stmod_cov.putr.unc)
...
```{r}
# extract posteriors
posteriors.upl.riceubcov <- insight::get_parameters(stmod_cov.rice)

# tidy
posteriors.upl.riceubcov.tidy <- posteriors.upl.riceubcov %>%
  pivot_longer(cols = starts_with("time"), names_to = "parameter", values_to = "values")
...
```{r}
posteriors.upl.riceubcov.tidy %>%
  ggplot() +
  geom_density(aes(fill = parameter, x = values),color="grey", alpha = .25) +
  # scale_fill_viridis(discrete = TRUE) +
  scale_fill_manual(values = colfunc2(ncol(posteriors.upl.riceubcov.tidy))) +
  theme_minimal() +
  geom_vline(aes(xintercept = 0), color= "black", size=1, lty="dashed") +
...
### Modeling and Posterior Description
```{r, cache=TRUE}
upl.rice.uc <- upl.rice %>% 
  filter(site_type == "core range") # add 

## add cover in 0-1 range
upl.rice.uc <- upl.rice.uc %>%
  mutate(cover = perc_cover/100)
...
```{r}
# extract posteriors
posteriors.upl.riceubcov.uc <- insight::get_parameters(stmod_cov.rice.uc)
...
### Modeling and Posterior Description
```{r, cache=TRUE}
upl.rice.unc <- upl.rice %>% 
  filter(site_type == "non-core range") # add 

## add cover in 0-1 range
upl.rice.unc <- upl.rice.unc %>%
  mutate(cover = perc_cover/100)
...
```{r}
# extract posteriors
posteriors.upl.riceubcov.unc <- insight::get_parameters(stmod_cov.rice.unc)
