...
For anayses of aspen count data, entailing separate models for combined winter range, core and noncore winter range subsets of plots, a random effects model was used for inference:

µi = exp(α + β1x1,i + β2x2,i + β3x3,i + γi) γi ∼ Normal(0, σ2)
yi ∼ Poisson(µi)
...
Interpreation of model coefficients: The exponentiated intercept (exp(α)) is the mean count
of aspen in unfenced plots during the baseline year; exp(β1) is the multiplicative change in mean counts that occurred during the time interval between the baseline year and 2013; exp(β2) is the multiplicative change in mean counts that occurred during the time interval between the baseline year and 2018; and exp(β2) is the multiplicative change in mean counts that was caused by fencing. Not exponentiating the coefficients makes them more difficult to interpret: α is mean of the log of counts in open plots in the first year and the coefficients are the additive change in the log of counts attributable to the year class or fence treatment.
...
fun.sexit.2plot(stmod_stally2.acanc.sm) %>%
  ...
  across(.cols = everything(), .fns = exp)
```
