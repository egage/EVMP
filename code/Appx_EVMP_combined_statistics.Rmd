...
plotF.emmip_FxTCxB_response <- function(model){
  emmip(model, fenced ~ time_class | fenced, type = "response") +
  theme_minimal() +
  scale_color_manual(values = colfunc3(2)) +
  labs(color="") +
  theme(legend.position = "top")
...
plotF.emmip_FxTC_response(wc.wnc.stmod_TCxF) +
  labs(title="Combined winter range") +
  theme(legend.position = "right")

ggsave("./output/figures_exported/emmip_TCxF_wcwnc.png", width = 3.75, height = 3.55) # save plot
```
...
# plotF.emmip_FxTCxB_response(wcwnc.emm.TCxFxB)

emmip(wcwnc.emm.TCxFxB, fenced ~ time_class | fenced * burned, 
      type = "response",
      CIs = TRUE) +
  labs(title="Combined winter range", caption = "willow height") +
  theme_minimal()


ggsave("./output/figures_exported/emmip_TCxFxB_wcwnc.png", width = 4.75, height = 4.25) # save plot
...
ggsave("./output/figures_exported/emmip_TCxF_wc.png", width = 3.75, height = 3.55) # save plot
```
...
```{r}
plotF.emmip_FxTC_response(stmod_ht_wk_TCxF) +
  labs(title="Combined winter range") +
  theme(legend.position = "right")

ggsave("./output/figures_exported/emmip_TCxF_wk.png", width = 3.75, height = 3.55)
```
...
canopy.all.willow.sum <- canopy.all.willow.sum %>% 
  mutate(time_class = fct_relevel(time_class, "2013", after = 1)) %>% 
  mutate(time_class = fct_relevel(time_class, "2015", after = 2)) %>% 
  mutate(time_class = fct_relevel(time_class, "2016", after = 3)) %>% 
  mutate(time_class = fct_relevel(time_class, "2017", after = 4))
...
```{r}
plotF.emmip_FxTC_response(stmod_cov.wc.wnc) +
  labs(title="Combined winter range", caption = "All willow cover") +
  theme(legend.position = "right") 

ggsave("./output/figures_exported/emmip_TCxF_wcwnc_cov.png", width = 5, height = 4.25)
...
emmip(stmod_cov.wc.wnc.TCxFxB, fenced ~ time_class |burned, 
      type = "response",
      CIs = TRUE) +
  scale_color_manual(values = colfunc3(2)) +
  labs(title="Combined winter range", caption = "willow cover, all willow species combined", color = "") +
  theme_minimal()


ggsave("./output/figures_exported/emmip_TCxFxB_wcwnc_cover_wide.png", width = 7.75, height = 4.25) # save plot
...
```{r}
plotF.emmip_FxTC_response(stmod_cov.wc) +
  labs(title="Core winter range", caption = "Combined willow cover, modeled means and CIs") +
  theme_minimal()

ggsave("./output/figures_exported/emmip_TCxF_wc_cov.png", width = 5, height = 4.25) # save plot
...
# emmip TEST before running

```{r}


emmip(stmod_cov.wnc, fenced ~ time_class, 
      type = "response",
      CIs = TRUE) +
  labs(title="Combined winter range", caption = "willow cover") +
  theme_minimal()


ggsave("./output/figures_exported/emmip_TC_wcwnc_ht.png", width = 4.75, height = 4.25) # save plot
...
# emmip #test before knit

```{r}


emmip(stmod_cov.wk, fenced ~ time_class | fenced, 
      type = "response",
      CIs = TRUE) +
  labs(title="Combined winter range", caption = "willow cover") +
  theme_minimal()
...
Gabray, J., Goodrich, B. 2020b. Estimating Generalized Linear Models for Count Data with rstanarm.
ver:2020-07-20 <URL:https://mc-stan.org/rstanarm/articles/count.html/>.

Gabray, J., Goodrich, B. 2020c. Modeling Rates/Proportions using Beta Regression with rstanarm.
version 2020-07-20 <URL:https://mc-stan.org/rstanarm/articles/betareg.html/>.
