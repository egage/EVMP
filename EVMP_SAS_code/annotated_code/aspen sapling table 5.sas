/*This program creates data found in table 5 of Zeigenfuss and Johnson (2015)--Least squares means of aspen sapling (plants with height less than or equal to 2.5 meters) density at aspen monitoring sites at sites
 burned in the 2012 Fern Lake Fire compared to unburned sites in Rocky Mountain National Park, Colorado. The first step of the code reads
...
/*This step performs some cleanup work is done on the remaining dataset with sapling-sized stems. Sites listed with the potential to be fenced "P" are now 
  ...
  because they relate to the EVMP objectives) and the mean stems per acre for fenced/unfenced, burned/unburned, core/non-core are calculated.*/
...
/*A new variable that combines all groupings (fenced, burned, core/non-core) as a single variable is created. Missing data points are removed. The log of 
...
data aspen2b;
    ...
    ldens=log(density+0.0001);
 	...
 	if sitetype='AC' and fenced='Y' and burned='Y' then newtype='ACFB';
	if sitetype='AC' and fenced='N' and burned='Y' then newtype='ACGB';
 	if sitetype='AC' and fenced='Y' and burned='N' then newtype='ACFN';
	if sitetype='AC' and fenced='N' and burned='N' then newtype='ACGN';
	if sitetype='ANC' and burned='N' then newtype='ANCN';
	if sitetype='ANC' and burned='Y' then newtype='ANCB';
...
/* This step produces the information used to determine what proportion of sites meets the various targets for recruitment (tall) and regeneration (short),
   information on statistical tests, and mean densities. LSMeans for table 5 are produced here. */


 data aspen3;
    set tran2;
    if recruit=0 then recruit_class=1;
    if recruit gt 0 and recruitlt 324 then recruit_class=2;
    if recruit ge 324 then recruit_class=3;
    if regen le 1700 then regen_class=1;
    if regen gt 1700 and regenle 4500 then regen_class=2;
